...
function targetStopsFor(minutes: number): number {
...
function pickStops(
  ...
  // Pull more adjacents as targetStops grows so we always have enough candidates.
  const adjacentPool = adjacent.slice(0, Math.max(6, targetStops * 3));
  const ranked = [...local, ...adjacentPool]
  ...
  // Pass 1: enforce category variety until we have at least 2 distinct categories.
  const picked: Place[] = [];
  ...
  // Pass 2: relax variety to fill remaining slots.
  for (const { p } of filtered) {
    if (picked.length >= targetStops) break;
    if (!picked.includes(p)) picked.push(p);
  }
  return picked.slice(0, targetStops);
}
...
export function generateDetour(input: GenerateInput): Detour {
  ...
  const targetStops = targetStopsFor(minutes);
  const places = pickStops(effectiveProfile, neighborhood, refinement, weather, targetStops);
  ...
  // Variant reflects both length and mode so the card visibly shifts as minutes change.
  let variant: string;
  if (stops.length <= 1) {
    variant = "Quick Stop";
  } else if (stops.length >= 4) {
    variant = mode === "bike" ? "Crawl" : "Long Wander";
  } else {
    variant = mode === "bike" ? "Loop" : mode === "walk" ? "Walk" : "Detour";
  }
  const title = `${titleBase} ${variant}`;

  const between = mode === "walk" ? 12 : 6;
  // Honor the actual stop count; minutes acts only as a true ceiling.
  const naturalDuration = stops.length * 25 + Math.max(0, stops.length - 1) * between;
  const durationMin = Math.min(minutes, naturalDuration);
