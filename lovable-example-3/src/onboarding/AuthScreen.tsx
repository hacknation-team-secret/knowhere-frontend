...
import { knowhereApi, ApiError, NetworkError, type ApiUser } from "@/lib/knowhereApi";
...
export function AuthScreen({ initialUsername, onAuthed, onSkip }: Props) {
  ...
  const [errorKind, setErrorKind] = useState<"network" | "other" | null>(null);
        ...
        <div className="mt-2 mb-2">
          ...
          {errorKind === "network" && (
            <p className="text-[12px] text-ink-soft mt-1.5 leading-relaxed">
              Some ad-blockers and the preview iframe occasionally block <span className="font-mono">railway.app</span>.
              Try once more, or open this in a new tab / incognito window if it persists.
            </p>
          )}
        </div>
      )}
