...
export function CityPicker({ city, onChange }: Props) {
  ...
  const { data: apiCities, isLoading } = useQuery({
    queryKey: ["cities"],
  ...
  const allCities = [
    "Boston",
    ...(apiCities?.map((c) => c.name) || []),
    ...CROSS_CITY_LIST.filter(c => !apiCities?.some(ac => ac.name === c))
  ];
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full z-20 mt-2 max-h-[60vh] overflow-y-auto rounded-2xl border border-border bg-card p-1.5 shadow-lift">
          ...
          {allCities.map((c) => {
            ...
            const isApi = apiCities?.some(ac => ac.name === c);
            return (
              <button
                key={c}
                onClick={() => {
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">
                    {isLive 
                      ? "Full pulse · routes · perks" 
                      : isApi 
                        ? "Real-time events & maps"
                        : "Curated single-stop picks"}
                  </p>
                </div>
